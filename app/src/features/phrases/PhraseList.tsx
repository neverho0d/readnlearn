...
export const PhraseList: React.FC<DictionaryViewProps> = ({
    filterText = "",
    ...
    const refresh = React.useCallback(async () => {
        ...
        try {
            ...
            // Prefer current file phrases when available to reduce data
            if (savedPhrases.length > 0 && !showAllPhrases) {
                // Convert savedPhrases to lightweight rows (keeps UI responsive)
                const rowsLite: PhraseRow[] = savedPhrases.map((p) => ({
                    id: p.id,
                    lang: settings.l2,
                    text: p.text,
